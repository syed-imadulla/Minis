...
function createGiftSparkles() {
  ...
  for (let i = 0; i < 120; i++) {
    ...
    sparkle.style.left =
      rect.left + rect.width / 2 + (Math.random() - 0.5) * 80 + "px";
    sparkle.style.top =
      rect.top + rect.height / 2 + (Math.random() - 0.5) * 60 + "px";
    ...
    sparkle.animate(
      [
        ...
        {
          opacity: 0,
          transform: `translate(${x}px,${y}px) scale(0.2) rotate(360deg)`,
        },
      ],
    ...
    if (voiceMessage) {
      voiceMessage.play().catch((err) => {
        ...
        if (audioCard && !audioCard.querySelector(".play-hint")) {
          ...
          hint.style.cssText =
            "font-size:0.8rem; margin-top:10px; opacity:0.7;";
...
function createHeartParticle() {
  ...
  heart.animate(
    [
      ...
      {
        transform: `translateY(-${200 + Math.random() * 300}px) scale(0.5) rotate(${Math.random() * 360}deg)`,
        opacity: 0,
      },
    ],
    {
...
function createConfetti() {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    confetti.style.left =
      window.innerWidth / 2 + (Math.random() - 0.5) * 400 + "px";
...
function adjustLetterSpacing() {
  ...
  if (revealContent && letterWrapper) {
    const observer = new MutationObserver(() => {
      if (
        giftPhoto.classList.contains("show") ||
        giftAudio.classList.contains("show")
      ) {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      heart.animate(
        [
          ...
          {
            transform: `translateY(-${80 + Math.random() * 100}px) scale(0.5)`,
            opacity: 0,
          },
        ],
        {
